...
class RequestedSystem(models.Model):
    ...
    # ✅ NEW: HOD Decision Date (Per System)
    ...
    # ✅ NEW: ICT Decision Date (Per System)
...
# ... [Keep CustomUserManager, CustomUser, UserRole, SystemAdminAssignment, SystemAdmin, HodAssignment, UserProfile] ...
...
class UserProfile(models.Model):
    ...
    hod = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, related_name="staff_members", limit_choices_to={'profile__role': 'hod'})
...
def auto_assign_hod(sender, instance, **kwargs):
    if instance.role == "staff" and instance.directorate:
        hod_profile = UserProfile.objects.filter(role="hod", directorate=instance.directorate).first()
        if hod_profile:
